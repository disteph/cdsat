...
module ProofSearch (F: FormulaImplem) (FSet: CollectImplem with type e = F.t) (ASet: CollectImplem with type e = Atom.t) =
    (struct
	 ...
	 let (index, word, modulo) =
	   match ans with
	     | Local(Success(_)) -> (0,"LocalSuccess",every.(0))
	     | Local(Fail(_))    -> (1,"LocalFail",every.(1)) 
	     | Fake(a,b,_)       -> let mydir = if b then "right" else "left" in
		 if a then (2,"FakeSuccess->"^mydir,every.(2)) else (3,"FakeFail->"^mydir,every.(3))
	 in
