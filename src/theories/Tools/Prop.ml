...
module Structure(F:Kernel.Formulae.FormulaType) = struct
  ...
  let symb_i (symb: [> ThSig_register.PropSig.symbol]) (formulalist:F.t list) = 
    match symb, formulalist with
      ...
      | _             -> raise (Theories.ModelError "ModelError: Not the right number of arguments")
