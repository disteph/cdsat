...
(* highest symbol in a term *)
  ...
  val predicate : Atom.t -> bool*string*(t list)
...
module Algo (X:SolvableTheory) (U:PersistentUnionFind with type e = X.v 
and type d = X.input) = struct
...
n : input list; phi : input list}

  exception Inconsistency of input list
...
(* find a subterm such that it is not in theta but all its direct subterms are *)
  ...
  and  aux theta = function
      | [] -> raise Finished
      | (a::ta) when (TSet.is_in a theta) -> aux theta ta
...
(* compute a step of the algorithm *)
  let rec step (s,sub,u) =
	...
	(TSet.is_in a s.theta)&&(TSet.is_in b s.theta)&&
	...
	| (Sol(p,q)) ->
	  ...
	  begin
		...
		if (VSet.is_in p (leaves (VtoV.find (make t) s.delta))) then
    ...
    | (NEq(a,b)) when (TSet.is_in a s.theta)&&(TSet.is_in b s.theta) ->
    ...
    | (IsEq(a,b)) when (TSet.is_in a s.theta)&&(TSet.is_in b s.theta) ->
      if ((VtoV.find (make a) s.delta)=(VtoV.find (make b) s.delta)) then
	(* rule QUERY *)
	...
	((*print_string "bad query "; (*print_term a; print_term b;*) print_newline();*)
	 raise Unknown)
    | (IsNEq(a,b)) when (TSet.is_in a s.theta)&&(TSet.is_in b s.theta) ->
      if ((VtoV.find (make a) s.delta)=(VtoV.find (make b) s.delta)) then
    ...
    | (Eq(a,b)) | (NEq(a,b)) | (IsEq(a,b)) | (IsNEq(a,b)) | (Congr(a,b)) ->
      ...
      let fs = findSterm (if not (TSet.is_in a s.theta) then a else b) 
	s.theta in
