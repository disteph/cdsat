...
let getIdent = function
  | IdSymbol(_,symb)                 -> getsymbSymbol symb,[]
  | IdUnderscoreSymNum(_,symb,(_,l)) -> getsymbSymbol symb,l

let rec getsort = function
  | SortIdentifier(_,ident)        -> Sort(getIdent ident,[])
  | SortIdSortMulti(_,ident,(_,l)) -> Sort(getIdent ident, List.map getsort l)

let getsortedvar (SortedVarSymSort(_,symb,sort)) =
  getsymbSymbol symb, getsort sort

type 'a getsymbType =
  | AppliedSymbol of ident*(term list)*('a environment)
...
and getsymbIdSymbol env tl = function
  ...
  | IdUnderscoreSymNum(_,symb,(_,param)) ->
    AppliedSymbol((getsymbSymbol symb, param),tl,env)
...
let guessThDecProc aft =
  ...
  | Some "QF_BV"  -> Some ["BV"; "bool"; "IfThenElse"]
