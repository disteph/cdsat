...
module ProofSearch(PlDS: PlugDSType) : sig
  ...
  module Make(DS: Specs.DSproj with type ts = PlDS.UF.t FormulaF.generic)
         : API with type FE.sassign = DS.Term.t * DS.Value.t Values.t
                and type FE.constraints = FirstOrder.Constraint.t
