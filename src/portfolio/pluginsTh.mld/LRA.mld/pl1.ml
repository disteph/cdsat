...
module Make(DS: GlobalImplem) = struct
  ...
  module Make(K: API.API with type sign   = MyTheory.sign
    ...
    let rec speak machine state =
      ...
      | None ->
         ...
         | None ->
            ...
            | None ->
               (* Some constraints still need to be satisfied somehow. Time to decide. *)
      ...
      | Some((c,b),_) ->
         ...
         match eval c with
         | Beval msg ->
            (* c of sort Bool actually evaluate - we send the propagation message *)
            Print.print ["LRA",4] (fun p -> p "LRA: kernel says %a" Msg.pp msg);
         ...
         | Unit{ var; nature; is_coeff_pos; rhs_cst = bound } ->
            ...
            | Range.FourierMotzkin(ba1,ba2) ->
               let msg = K.fm ba1 ba2 var in
               Msg msg, machine { state with watchedB }
            | Range.DisEqual(ba1,ba2,ba3) -> failwith "TODO: DisEqual"
    ...
    let rec machine state =
      ...
      let share tset =
        let new2evaluate, kernel = K.share tset state.kernel in
