...
module Make
  ...
  (U : PersistentUnionFind with type e = X.v 
                           and  type d = X.input) 
  : Tools.ForGround.Type = 
struct
    
  module Atom = X.Atom

  let names   = ["emptyCC";"QF_UF"]
  let sugPlugin = None
  ...
  struct
    ...
    let goal_consistency atomN t =
      if ASet.is_in t atomN then Some (ASet.add t ASet.empty)
        ...
        begin
          Dump.msg (Some(fun p->p "Procedure called on goal %a under hypotheses\n%a" Atom.print_in_fmt (EAtom.proj t) ASet.print_in_fmt atomN)) None None;
	  let l = ASet.fold (fun t l -> (EAtom.proj t)::l) atomN [] in
	  ...
	  let (b,p,s) = X.predicate (EAtom.proj t) in
          let result = 
            match p with
            | `Eq _ ->
              begin
	        let q = Some([Alg.toQuery (X.atoI (EAtom.proj t))]) in
