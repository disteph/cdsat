...
module GenPlugin(ThDS: TheoryDSType)
  :(Plugin.Type  with type DS.UASet.e = ThDS.IAtom.t
                 and  type DS.UF.lit  = ThDS.Atom.t) = 
struct
    
  module DS = DataStructures.Generate(ThDS)
  ...
  *)
  ...
  module Strategy(FE:FrontEndType with type ASet.e      = DS.UASet.e
    ...
    module Me = Gplugins_tools.Memoisation.Make(IAtom)(FE)(UFSet)(UASet)
    ...
    module H = Hashtbl.Make(Gplugins_tools.Utils.PHCons_ext(IAtom))
    ...
    *)
	...
	| CSet.Leaf(j,x)       ->
	  ...
	  | Yes _   ->
	    ...
	    (Some j,t)
	  | Almost n when not (UASet.mem (IAtom.negation n) atms) ->
    ...
    *)
	...
	(let lit = pick_lit_from tset in
         ...
         if UASet.mem (IAtom.negation lit) atms then failwith "Chosen nlit in atms";
         count.(3)<-count.(3)+1;
         Some(ACut(lit,branch_one adO,accept,accept,fNone)))
      else
        None
    ...
    *)
	...
	| (f,n,_)::h when not (UASet.mem (IAtom.negation n) atms)->
	    (count.(2)<-count.(2)+1;
	     stack:=h;
