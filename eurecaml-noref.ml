...
type seq = 
    EntF of ((string*(term list)) zipper)*formula*(posFormula zipper)*(posFormula list)
  | EntUF of ((string*(term list)) zipper)*(formula zipper)*(posFormula zipper)*(posFormula list);;
...
let rec printseq = function
    ...
    -> " \\DerOSNeg {"^
      (printzipper (function (s, tl) -> s^"("^printtl(tl)^")") " \\cdot " atomsN)^
      "} {"^(printzipper printformula " \\cdot " unfocused)^"}"^
      "{"^(printzipper printformulaP " \\cdot " formuP)^"\\mid "^
      (printl true printformulaP formuPSaved)^"}";
      
  | EntF(atomsN, focused, formuP, formuPSaved)
    -> " \\DerOSPos {"^(printzipper (function (s, tl) -> s^"("^printtl(tl)^")") " \\cdot " atomsN)^
...
let rec lk_solve cont inloop = function
    ...
    -> let (newdelta, toDecompose) = next (delta) in
      begin
        match toDecompose with
	      ...
	      else lk_solve newcont false (EntUF (gammatomN, newdelta, add_right p gammaformP,gammaformPSaved))
	      ...
	      then lk_solve newcont inloop (EntUF (gammatomN,newdelta, gammaformP, gammaformPSaved))
	      else lk_solve newcont false (EntUF (add_right (f, tl) gammatomN,newdelta, gammaformP, gammaformPSaved))
	  ...
	  | Und(PosAtomU(f, tl)) 
	    -> lk_solve cont false (EntUF(add_right (f, tl) gammatomN, applyzipper (forcepolF f false) newdelta, applyzipper (forcepolP f false) gammaformP, applylist (forcepolP f false) gammaformPSaved))
	  | Und(NegAtomU(f, tl)) 
	    -> lk_solve cont false (EntUF(add_right (f, tl) gammatomN, applyzipper (forcepolF f true) newdelta, applyzipper (forcepolP f true) gammaformP, applylist (forcepolP f true) gammaformPSaved))
	  | Und(a) 
  ...
  | EntF(gammatomN, Und(PosAtomU(f, tl)), gammaformP, gammaformPSaved) 
    -> lk_solve cont false (EntF(gammatomN, Neg(NegAtom(f, tl)), applyzipper (forcepolP f false) gammaformP, applylist (forcepolP f false) gammaformPSaved))
  | EntF(gammatomN, Und(NegAtomU(f, tl)), gammaformP, gammaformPSaved) 
    -> lk_solve cont false (EntF(gammatomN, Neg(NegAtom(f, tl)), applyzipper (forcepolP f true) gammaformP, applylist (forcepolP f true) gammaformPSaved))
